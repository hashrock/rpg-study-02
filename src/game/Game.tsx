...
function TownView({ state, onEnterCave, onHire }: {
  state: GameState
  onEnterCave: () => void
...
function BattleView({ state, onAllyAction, onEnemyAuto }: {
  state: GameState
  onAllyAction: (targetIndex: number) => void
  onEnemyAuto: () => void
}) {
  const battle = state.battle!
  const actor = battle.turnOrder[battle.turnIndex]
  const isAllyTurn = actor.kind === 'ally' && battle.allies[actor.index].hp > 0
  const isEnemyTurn = actor.kind === 'enemy' && battle.enemies[actor.index].hp > 0
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '1rem', minHeight: 80 }}>
        {isAllyTurn && (
          <div>
            <div>行動選択：{battle.allies[actor.index].name}</div>
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              {battle.enemies.map((e, i) => (
                <button key={`t-${i}`} disabled={e.hp <= 0} onClick={() => onAllyAction(i)}>
                  こうげき → {e.name}
                </button>
              ))}
            </div>
      ...
      <div style={{ marginTop: '1rem', textAlign: 'left', maxWidth: 640, marginInline: 'auto' }}>
        <h4>ログ</h4>
        <div style={{ border: '1px solid #444', padding: '0.5rem', borderRadius: 8, minHeight: 80 }}>
...
export default function Game() {
  ...
  function handleHire(c: Character) {
    setState((prev) => {
      ...
      if ([prev.party.hero, ...prev.party.companions].some((m) => m.id === c.id)) return prev
      return {
        ...prev,
        party: {
          ...prev.party,
  ...
  function runEnemyTurn() {
    setState((prev) => {
      ...
      const b = { ...prev.battle, allies: prev.battle.allies.map((a) => ({ ...a })), enemies: prev.battle.enemies.map((e) => ({ ...e })) }
      ...
      b.log = [...b.log, `${attacker.name} のこうげき！ ${target.name} に ${dmg} ダメージ！`]
  ...
  function runAllyAttack(targetIndex: number) {
    setState((prev) => {
      if (prev.mode !== 'BATTLE' || !prev.battle) return prev
      const b = { ...prev.battle, allies: prev.battle.allies.map((a) => ({ ...a })), enemies: prev.battle.enemies.map((e) => ({ ...e })) }
      const actor = b.turnOrder[b.turnIndex]
  ...
  return (
    <div>
      ...
      {state.mode === 'BATTLE' && state.battle && (
        <BattleView state={state} onAllyAction={runAllyAttack} onEnemyAuto={runEnemyTurn} />
      )}
      {state.mode === 'CLEAR' && (
        <div>
          <h2>クリア！</h2>
